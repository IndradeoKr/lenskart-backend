...
public class AdminServiceImpl implements IAdminService {
	...
	public String addAdmin(UserDTO adminDTO) throws LenskartApplicationException {
		...
		if (!userRepository.findByRoleIgnoreCase(ADMIN_ROLE).isEmpty()) {
			throw new LenskartApplicationException("Only one admin is allowed");
		}
	...
	public String updateAdminName(String email, String password, String name) throws LenskartApplicationException {
		...
		if (existing == null) {
			...
			throw new LenskartApplicationException("Admin with email " + email + " not found.");
		}
		...
		if (!(passwordEncoder.matches(password, existing.getPassword()) || existing.getPassword().equals(password))) {
			...
			throw new LenskartApplicationException("Admin with email " + email + " Wrong Password.");
		}
		...
		if (!ADMIN_ROLE.equalsIgnoreCase(existing.getRole())) {
			...
			throw new LenskartApplicationException("User with email " + email + " is not an admin.");
		}
	...
	public String deleteAdminById(int adminId) throws LenskartApplicationException {
		...
		if (user == null) {
			...
			throw new LenskartApplicationException("Admin with ID " + adminId + " not found.");
		}
		...
		if (!ADMIN_ROLE.equalsIgnoreCase(user.getRole())) {
			logger.warn("User with ID: {} and email: {} is not an admin. Role: {}", user.getUserid(), user.getEmail(),
					user.getRole());
			throw new LenskartApplicationException("User with email " + user.getEmail() + " is not a admin.");
		}
	...
	@Override
	public List<UserDTO> getAllAdmins() throws LenskartApplicationException {
