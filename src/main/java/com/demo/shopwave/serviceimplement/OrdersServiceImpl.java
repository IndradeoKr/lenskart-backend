...
import com.demo.shopwave.dto.OrdersDTO;
import com.demo.shopwave.dto.OrdersDTO.Status;
...
import com.demo.shopwave.repository.OrdersRepository;
import com.demo.shopwave.service.IOrdersService;
...
public class OrdersServiceImpl implements IOrdersService {
	...
	public ResponseEntity<String> addOrders(OrdersDTO ordersDTO) {
		...
		Cart cart = cartRepository.findById(ordersDTO.getCartId()).orElseThrow(() -> {
			...
			return new ShopwaveApplicationException("Cart not found with this ID");
		});
		...
		if (orderQuantity <= 0) {
			...
			throw new ShopwaveApplicationException("order cant't be placed due to unavailability");
		}
		...
		if (ordersDTO.getDate() == null) {
			...
			throw new ShopwaveApplicationException("Invalid order date");
		}
	...
	public ResponseEntity<OrdersDTO> updateOrders(OrdersDTO ordersDTO) {
		...
		if (ordersDTO.getOrderId() <= 0) {
			...
			throw new ShopwaveApplicationException("Invalid Order ID");
		}
		...
		if (order == null) {
			...
			throw new ShopwaveApplicationException("Order with ID " + ordersDTO.getOrderId() + " not found");
		}

		if (ordersDTO.getDate() == null) {
			logger.error("Missing date for Order ID: {}", ordersDTO.getOrderId());
			throw new ShopwaveApplicationException("Order date cannot be null");
		}

		if (ordersDTO.getStatus() != null) {
			if (order.getStatus() == Orders.Status.DELIVERED && ordersDTO.getStatus() == OrdersDTO.Status.IN_PROGRESS) {
				throw new ShopwaveApplicationException("Cannot change status from DELIVERED to IN_PROGRESS");
			}
	...
	public ResponseEntity<List<OrdersDTO>> findAll() {
		...
		if (ordersList.isEmpty()) {
			...
			throw new ShopwaveApplicationException("No orders found in the system");
		}
	...
	public ResponseEntity<List<OrdersDTO>> getOrderCustomerId(int customerId) {
		...
		if (customerId <= 0) {
			...
			throw new ShopwaveApplicationException("Customer ID must be a positive integer");
		}
		...
		if (ordersList == null || ordersList.isEmpty()) {
			...
			throw new ShopwaveApplicationException("No orders found for customer ID: " + customerId);
		}
